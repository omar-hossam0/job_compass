...
async function testCVMatching() {
  try {
    ...
    if (response.data.success) {
      ...
      if (response.data.criticalSkills && response.data.criticalSkills.length > 0) {
        console.log(`\n🎯 Critical Skills Identified:`);
        console.log(`   ${response.data.criticalSkills.slice(0, 15).join(', ')}`);
        if (response.data.criticalSkills.length > 15) {
          console.log(`   ... and ${response.data.criticalSkills.length - 15} more`);
        }
      }
      ...
      if (candidates.length === 0) {
        ...
        candidates.slice(0, 10).forEach((candidate, idx) => {
          const matchIcon = candidate.matchScore >= 75 ? '🟢' :
                           candidate.matchScore >= 60 ? '🟡' : '🔴';
          const matchLabel = candidate.matchScore >= 75 ? 'Excellent' :
                            candidate.matchScore >= 60 ? 'Good' :
                            candidate.matchScore >= 45 ? 'Fair' : 'Low';
          
          console.log(`\n   ${idx + 1}. ${matchIcon} ${candidate.name}`);
          console.log(`      Email: ${candidate.email}`);
          console.log(`      ✅ Match Score: ${candidate.matchScore}% (${matchLabel})`);
          
          // Show breakdown if available
          if (candidate.semanticScore !== undefined && candidate.keywordScore !== undefined) {
            console.log(`      📊 Breakdown:`);
            console.log(`         - Semantic Similarity: ${candidate.semanticScore}%`);
            console.log(`         - Keyword Matching: ${candidate.keywordScore}%`);
          }
          
          if (candidate.matchedSkills !== undefined && candidate.totalSkills !== undefined) {
            console.log(`      🎯 Skills Matched: ${candidate.matchedSkills}/${candidate.totalSkills}`);
          }
        ...
        // Statistics
        const avgScore = (candidates.reduce((sum, c) => sum + c.matchScore, 0) / candidates.length).toFixed(2);
        const excellentMatches = candidates.filter(c => c.matchScore >= 75).length;
        const goodMatches = candidates.filter(c => c.matchScore >= 60 && c.matchScore < 75).length;
        const fairMatches = candidates.filter(c => c.matchScore >= 45 && c.matchScore < 60).length;
    ...
    console.error('❌ Error during matching:', error.response?.data || error.message);
    if (error.code === 'ECONNABORTED') {
      console.error('   ⏱️  Request timed out. Python script may be taking too long.');
    }
    return false;
  }
}
