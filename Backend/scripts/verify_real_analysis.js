      ...
      python.on("close", (code) => {
        if (code !== 0) {
          ...
          try {
            ...
            reject(
              new Error(`Failed to parse output: ${e.message}\n${outputData}`),
            );
          }
        }
      });
...
async function analyzeResults(results, scenario) {
  ...
  if (results.critical_skills) {
    console.log(
      `Top Skills: ${results.critical_skills.slice(0, 10).join(", ")}`,
    );
  }
  ...
  results.matches.forEach((match, idx) => {
    ...
    console.log(`\n${icon} CV #${cvNum}: ${score}%`);
    console.log(
      `   Breakdown: Semantic=${semanticScore}% + Keywords=${keywordScore}%`,
    );
    console.log(`   Skills Matched: ${matchedSkills}/${totalSkills}`);

    // Verify the math
    const calculatedScore =
      Math.round((semanticScore + keywordScore) * 100) / 100;
    if (Math.abs(calculatedScore - score) > 0.1) {
      console.log(
        `   ⚠️  WARNING: Math doesn't match! ${calculatedScore} vs ${score}`,
      );
    } else {
      console.log(
        `   ✅ Math verified: ${semanticScore} + ${keywordScore} = ${score}`,
      );
    }
  });
...
async function verifyDifferentJobsProduceDifferentResults() {
  ...
  if (allResults.length >= 2) {
    ...
    const cv1Scores = allResults.map(
      (r) => r.matches[0]?.similarity_score || 0,
    );
    ...
    const variance =
      cv1Scores.reduce((sum, score) => sum + Math.pow(score - mean, 2), 0) /
      cv1Scores.length;
...
async function testWithRealDatabase() {
  ...
  try {
    ...
    if (
      ...
      jobsResponse.data.data.length === 0
    ) {
      console.log(
        "⚠️  No jobs found in database. Creating test jobs would be needed.",
      );
      return;
    }
    ...
    if (matchResponse.data.success) {
      ...
      if (matchResponse.data.criticalSkills) {
        console.log(
          `   Critical Skills: ${matchResponse.data.criticalSkills.slice(0, 10).join(", ")}`,
        );
      }
      ...
      candidates.slice(0, 5).forEach((candidate, idx) => {
        ...
        if (candidate.semanticScore && candidate.keywordScore) {
          console.log(
            `   Breakdown: ${candidate.semanticScore}% + ${candidate.keywordScore}%`,
          );
          ...
          if (Math.abs(calculated - candidate.matchScore) <= 0.1) {
            ...
            console.log(
              `   ⚠️  Math mismatch: ${calculated} vs ${candidate.matchScore}`,
            );
          }
        }
        ...
        if (candidate.matchedSkills && candidate.totalSkills) {
          console.log(
            `   Skills: ${candidate.matchedSkills}/${candidate.totalSkills}`,
          );
        }
      });
      ...
      if (scores.length > 0 && uniqueScores.size === 1) {
        console.log(
          `❌ SUSPICIOUS: All ${scores.length} candidates have the SAME score (${scores[0]}%)`,
        );
        console.log(`   This might indicate fake data!`);
      } else if (uniqueScores.size > scores.length * 0.5) {
        console.log(
          `✅ GOOD: High score diversity (${uniqueScores.size} unique scores)`,
        );
        console.log(`   Model appears to be analyzing each CV individually.`);
      } else {
        console.log(
          `⚠️  Moderate diversity: ${uniqueScores.size} unique scores from ${scores.length} CVs`,
        );
      }
      ...
      if (scores.length >= 2) {
        ...
        if (diff < 5) {
          ...
          console.log(
            `   ✅ Good range - clear differentiation between candidates`,
          );
        } else {
...
async function runFullVerification() {
  ...
  console.log(
    "   1. Test Python model directly with different job descriptions",
  );
  ...
  console.log("💡 Summary:");
  console.log(
    "   ✅ If you see different scores for different jobs → Model is REAL",
  );
  console.log(
    "   ✅ If math checks out (semantic + keywords = total) → Calculations are CORRECT",
  );
